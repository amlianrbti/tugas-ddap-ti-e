import 'package:flutter/material.dart';
import '../../core/language_controller.dart';
import '../../widgets/galery/carousel_arrow.dart';
import '../../widgets/galery/carousel_card.dart';
import '../../widgets/galery/divider_with_text.dart';

class KegiatanSection extends StatelessWidget {
  final bool isMobile;
  final int kegiatanIndex;
  final List<Map<String, String>> kegiatanList;
  final Function(int) onChanged;

  const KegiatanSection({
    super.key,
    required this.isMobile,
    required this.kegiatanIndex,
    required this.kegiatanList,
    required this.onChanged,
  });

  @override
  Widget build(BuildContext context) {
    final double screenWidth = MediaQuery.of(context).size.width;
    final bool isTablet = screenWidth >= 600 && screenWidth < 1100;
    final double carouselHeight = isMobile ? 230 : (isTablet ? 305 : 435);

    return Container(
      padding: EdgeInsets.only(
        top: isMobile ? 24 : 48,
        bottom: isMobile ? 32 : 64,
      ),
      color: const Color(0xFFF3EAE0),
      child: Column(
        children: [
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 24),
            child: Text(
              'KEGIATAN'.tr,
              style: TextStyle(
                fontFamily: 'BrewedCoffee',
                fontSize: isMobile ? 24 : 36,
                color: const Color(0xFF000000),
                letterSpacing: 1.0,
              ),
            ),
          ),
          SizedBox(height: isMobile ? 12 : 24),
          Stack(
            alignment: Alignment.center,
            children: [
              SizedBox(
                height: carouselHeight,
                child: OverflowBox(
                  maxWidth: double.infinity,
                  maxHeight: double.infinity,
                  alignment: Alignment.center,
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: List.generate(3, (index) {
                      final int dataIndex =
                          (kegiatanIndex - 1 + index + kegiatanList.length) % kegiatanList.length;
                      final bool isActive = index == 1;

                      return CarouselCard(
                        title: kegiatanList[dataIndex]['title']!.tr,
                        image: kegiatanList[dataIndex]['image']!,
                        isActive: isActive,
                        screenWidth: screenWidth,
                        onTap: () {
                          onChanged(dataIndex);
                        },
                      );
                    }),
                  ),
                ),
              ),
              Positioned(
                left: isMobile ? 12 : 24,
                child: CarouselArrow(
                  icon: Icons.arrow_back_ios_new,
                  isMobile: isMobile,
                  onPressed: () {
                    onChanged(
                      (kegiatanIndex - 1 + kegiatanList.length) %
                          kegiatanList.length,
                    );
                  },
                ),
              ),
              Positioned(
                right: isMobile ? 12 : 24,
                child: CarouselArrow(
                  icon: Icons.arrow_forward_ios,
                  isMobile: isMobile,
                  onPressed: () {
                    onChanged(
                      (kegiatanIndex + 1) % kegiatanList.length,
                    );
                  },
                ),
              ),
            ],
          ),
          SizedBox(height: isMobile ? 30 : 50),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 24),
            child: DividerWithText(
              text: kegiatanList[kegiatanIndex]['subtext']!.tr,
              isMobile: isMobile,
            ),
          ),
        ],
      ),
    );
  }
}